...
Documentation is very subjective and sphinx is very customizable.
This section provides directions to make the documentation look
like the [shapes documentation](http://shapes.readthedocs.io/en/latest/?badge=latest).
However, this would provide a good starting point to work from.
...
This file is automatically created by sphinx.

Update the toctree Content section to include all other `rst` files such as intro and functions:
...
You can automatically add python documentation with the command:

`.. autofunction:: shapes.square.area.area_square`

##### [`conf.py`](docs/source/conf.py)

This file is also automatically created with sphinx. Below are the details for how the autogenerated file was changed.
